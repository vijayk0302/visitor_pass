...
const Passes = () => {
    ...
    const filterpass = passes.filter((u) => {
        const query = search.toLowerCase();
        return (u.issuedBy?.name?.toLowerCase().includes(query) ||
            u.validTo?.toLowerCase().includes(query) ||
    ...
    const totalPages = Math.ceil(filterpass.length / listperpage);
    const lastindex = currentpage * listperpage;
    const firstindex = lastindex - listperpage;
    const currentlists = filterpass.slice(firstindex, lastindex)
                            ...
                            {filterpass.length === 0 ? (<tr>
                                ...
                                currentlists.map((v) => (
                                    <tr key={v._id} className="border-t border-white/5 hover:bg-white/5 transition">
                                        ...
                                        <td className="px-6 whitespace-nowrap py-4"> {v.issuedBy.name} </td>
